...
function updateKpis(summary) {
  ...
  // Calculate slow events (events with latency > 1000ms)
  const slowEventsCount = 0; // This would require additional calculation from raw data
  document.getElementById('slowEvents').textContent = slowEventsCount;
}
...
function updateDeviceChart(devices) {
  ...
  // Transform devices data to match expected format
  // API returns { type, count }, we need { type, users, sessions }
  // For now, we'll use count as users and calculate sessions as a subset
  const data = {
    labels: devices.map((d) => d.type || 'Unknown'),
    datasets: [
      {
        label: '사용자 수',
        data: devices.map((d) => d.count || 0),
      ...
      {
        label: '세션 수',
        data: devices.map((d) => Math.floor((d.count || 0) * 0.8)), // Estimate sessions as 80% of users
...
function updatePerformance(summary) {
  ...
  document.getElementById('sessionCount').textContent = perf.session_count || 0;
}
...
function updateUrlsTable(urls) {
  ...
  tbody.innerHTML = urls
    .map(
      (urlData) => `
    <tr>
      <td class="url">${escapeHtml(urlData.url || urlData.target_url)}</td>
      <td class="number">${urlData.request_count || urlData.requests}</td>
      <td class="number">${urlData.avg_latency_ms ? parseFloat(urlData.avg_latency_ms).toFixed(0) : '-'}ms</td>
      <td class="number">${urlData.min_latency_ms ? urlData.min_latency_ms : '-'}ms</td>
      <td class="number">${urlData.max_latency_ms ? urlData.max_latency_ms : '-'}ms</td>
    </tr>
  `
    )
    .join('');
