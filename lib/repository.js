...
// 이벤트 로깅
function logEvent({ sessionId, eventType, targetUrl, latencyMs, locale }) {
  const db = getDb();
  const nowIso = new Date().toISOString();
...
function getHourlyStats() {
  ...
  stats.forEach(stat => {
    fullStats[stat.hour].count = stat.count;
  });
...
function getTotalChecks() {
  ...
  const result = db.get(sql`
    ...
    FROM events
    WHERE event_type = 'time_check'
  `);
  return result?.count || 0;
}

// 고유 URL 개수
function getUniqueUrlsCount() {
  const db = getDb();
  const result = db.get(sql`
    SELECT COUNT(DISTINCT target_url) as count
    FROM events
    WHERE target_url IS NOT NULL AND target_url != ''
  `);
  return result?.count || 0;
