...
// 페이지별 댓글 조회
async function getCommentsByPage(pageId, options = {}) {
    const db = getDb();
...
// 댓글 개수 조회
async function getCommentsCount(pageId) {
    const db = getDb();
...
// IP별 최근 댓글 개수
async function getRecentCommentCountByIp(ipHash, minutesAgo = 5) {
    const db = getDb();
...
async function cleanupOldComments(pageId, keepCount = 10) {
    const db = getDb();

    // 해당 페이지의 댓글 개수 확인
    const total = await getCommentsCount(pageId);
    ...
    const oldComments = await db.select({ commentId: comments.commentId })
        .from(comments)
        .where(and(
            eq(comments.pageId, pageId),
            eq(comments.isDeleted, 0)
